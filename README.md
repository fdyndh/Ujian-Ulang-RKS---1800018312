# Ujian-Ulang-RKS <h1> Aplikasi Cek Resi JNE 
## Bahwa front end yg dibangun mengikuti/sesuai dengan rancangan wireframe, workflow, dan dokumentasi kebutuhan yg telah dibuat sebelum UAS. Tampilan FE dibuat menggunakan android studi dan berbasis mobile.<h2>
###### Berikut saya sertakan tautan folder UTS sebelumnya yg berisi workflow, wireframe, dan dokumentasi kebutuhan.
https://drive.google.com/drive/folders/1aJgxd9LTptPBdvFrdvLv-aqMwHD0f1US?usp=sharing <h6>
 
###### Berikut file - file yang digunakan dalam pembuatan aplikasi cek resi jne : <h6>
 
 # A. Layout : <h3>
 ### - activity_buat_akun.xml <h3> 
 ### - activity_cek_resi.xml <h3>
 ### - activity_cek_resi_detail.xml <h3>
 ### - activity_main.xml <h3>
 ### - activity_otp.xml <h3>
 ### - activity_pass_code.xml <h3>
 ### - activity_welcome.xml <h3>
 ### - app_bar_main.xml <h3>
 ### - content_main.xml <h3>
 ### - fragment_home.xml <h3>
 ### - fragment_login.xml <h3>
 ### - fragment_profil.xml <h3>
 ### - fragment_riwayat.xml <h3>
 ### - nav_header_main.xml <h3>
 ### - opening.xml <h3>
 ### - slide_kedua.xml <h3>
 ### - slide_ketiga.xml <h3>
 ### - slide_pertama.xml <h3>
 
 # B.  Java : <h3>  
 ## - LoggedInUser.java <h3>
 ## - LoginDataSource.java <h3>
 ## - LoginRepository.java <h3>
 ## - Result.java <h3>
 ## - Home Fragment.java <h3>
 ## - HomeViewModel.java <h3>
 ## - LoggedInUserView.java <h3>
 ## - LoginForm.java <h3>
 ## - LoginFormState.java <h3>
 ## - LoginFragment.java <h3>
 ## - LoginResult.java <h3>
 ## - LoginviewModel.java <h3>
 ## - LoginviewModelFactory.java <h3>
 ## - ProfilFragment.java <h3>
 ## - ProfilviewModel.java <h3>
 ## - RiwayatFragment.java <h3>
 ## - RiwayatviewModel.java <h3>
 ## - BuatAkun.java <h3>
 ## - CekResi.java <h3>
 ## - CekResiDetail.java <h3>
 ## - InputOtp.java <h3>
 ## - MainActivity.java <h3>
 ## - PassCode.java <h3>
 ## - PrefManager.java <h3>
 ## - WelvomeActivity.java <h3>
 
 # C. Drawable : <h3>
 ## - chat.png <h3>
 ## - digital_wallet.png <h3>
 ## - ewallet.png <h3>
 ## - ic_baseline_account_circle_24.xml <h3>
 ## - ic_baseline_call_24.xml <h3>
 ## - ic_baseline_email_24.xml <h3>
 ## - ic_baseline_qr_code_scanner_24.xml <h3>
 ## - ic_baseline_search_24.xml <h3>
 ## - ic_baseline_share_24.xml <h3>
 ## - ic_launcher_background.xml <h3>
 ## - ic_top_up_24.xml <h3>
 ## - side_nav_bar.xml <h3>
 ## - jne.png <h3>
 ## - logo.png <h3>
 ## - more.png <h3>
 ## - myjne.png <h3>
 ## - myjob.png <h3>
 ## - nearby.png <h3>
 ## - token.png <h4>
 ## - price_tags.png <h3>
 ## - shipment.png <h3>
 ## - shipping.png <h3>
 
 # D. Values : <h3>
 ## - colors.xml <h3>
 ## - dimens.xml <h3>
 ## - drawables.xml <h3>
 ## - strings.xml <h3>
 ## - style.xml <h3>
 ## - themes.xml <h3>
 
 # E. Manifest : <h3>
 ## - AndroidManifest.xml <h3>
 
 ###### Berikut saya sertakan video gif aplikasi cek resi jne <h6>
![gif aplikasi cek resi jne](https://user-images.githubusercontent.com/44845450/107865585-8debec80-6e9a-11eb-9722-6610def67142.gif)
